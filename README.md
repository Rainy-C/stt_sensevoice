...
> **本插件仅为 SenseVoice 的客户端，不包含语音识别模型本身。使用前必须自行部署 SenseVoice 服务！**
>
> **部署方式请参阅下文 [SenseVoice 服务部署](#sensevoice-服务部署) 章节，推荐 Docker 一键部署。**
...
**本插件需要外部 SenseVoice 服务支持，请确保已完成以下步骤：**
1. 已部署 SenseVoice API 服务（Docker 或本地部署）
2. 服务可访问（默认端口 50000）
3. 已安装 ffmpeg（用于音频格式转换）
